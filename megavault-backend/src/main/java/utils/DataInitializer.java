...
public class DataInitializer implements CommandLineRunner {
    ...
    @Autowired
    public DataInitializer(ProductRepository productRepository, CategoryRepository categoryRepository, UserRepository userRepository, PasswordEncoder passwordEncoder) {
    ...
    public void run(String... args) throws Exception {
        // 1. Seed user registrations for authentication
        userRepository.deleteAll();

        User tejAdmin = User.builder()
                .name("Tej (Super Admin)")
                .email("tejs59885@gmail.com")
        ...
        User superAdmin = User.builder()
                .name("Sai Lajala (Admin)")
                .email("sainalajala984@gmail.com")
                .password(passwordEncoder.encode("admin123"))
                .role("ROLE_SUPER_ADMIN")
                .managedCategory("ALL")
                .isVerified(true)
        ...
        User customer = User.builder()
                ...
                .role("ROLE_CUSTOMER")
                .managedCategory("NONE")
                .isVerified(true)
                .build();

        User legacyCustomer = User.builder()
                .name("Sai Tej")
                .email("customer@megavault.com")
        ...
        userRepository.saveAll(Arrays.asList(tejAdmin, superAdmin, legacyAdmin, customer, legacyCustomer));
        System.out.println("✅ DataInitializer: Successfully seeded Super Admin (tejs59885@gmail.com) and Customer (sai9840tej@gmail.com)!");

        // 2. Seed 6 Structured Categories with numeric IDs ("1", "2", "3", "4", "5", "6")
        categoryRepository.deleteAll();
        List<Category> categories = Arrays.asList(
            Category.builder().catId("1").name("Audio & Sound").icon("bi-headphones").description("Headphones, TWS earbuds, speakers & soundbars").build(),
            Category.builder().catId("2").name("Smart Wearables").icon("bi-smartwatch").description("Smartwatches, fitness bands & health rings").build(),
            Category.builder().catId("3").name("Gaming & Gear").icon("bi-controller").description("Mechanical keyboards, gaming mice & monitors").build(),
            Category.builder().catId("4").name("Electronics & Tech").icon("bi-laptop").description("Laptops, 4K monitors, power banks & tablets").build(),
            Category.builder().catId("5").name("Fashion & Wear").icon("bi-bag").description("Backpacks, sunglasses, leather wallets & footwear").build(),
            Category.builder().catId("6").name("Smart Home").icon("bi-house-gear").description("Smart lighting, air purifiers & robot vacuums").build()
        );
        categoryRepository.saveAll(categories);
        System.out.println("✅ DataInitializer: Successfully seeded 6 Categories with Category IDs (1..6)!");

        // 3. Seed 30 Structured Products with Category-based Product IDs (101..105, 201..205, 301..305, 401..405, 501..505, 601..605)
        ...
        System.out.println("✅ DataInitializer: Successfully seeded all 30 Products with Category-based Product IDs (101..105, 201..205, 301..305, 401..405, 501..505, 601..605)!");
    }
}
